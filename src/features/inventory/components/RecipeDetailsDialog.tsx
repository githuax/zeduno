...
const RecipeDetailsDialog: React.FC<RecipeDetailsDialogProps> = ({
  ...
  const calcPours = (ing: any): number | null => {
    const vpu = ing.ingredientId?.volumePerUnitMl;
    const dps = ing.ingredientId?.defaultPourSizeMl;
    const trackBy = ing.ingredientId?.trackBy;
    const stock = ing.ingredientId?.currentStock;
    if (trackBy === 'volume' && vpu && dps && dps > 0 && stock && stock > 0) {
      return Math.floor((stock * vpu) / dps);
    }
    return null;
  };
                      ...
                      return (
                        <TableRow key={index}>
                          ...
                          <TableCell>
                            ...
                            {pours !== null && (
                              <div className="text-xs text-muted-foreground">≈ {pours} pours</div>
                            )}
                          </TableCell>
                          <TableCell>
