...
const EditMenuItemForm: React.FC<EditMenuItemFormProps> = ({
  ...
  const [estimatedPours, setEstimatedPours] = useState<number | null>(null);
  const [loadingPours, setLoadingPours] = useState<boolean>(false);

  const { toast } = useToast();
  const { currencyCode, symbol } = useCurrency();

  // Fetch recipe and estimate pours remaining for this menu item
  useEffect(() => {
    const load = async () => {
      try {
        setLoadingPours(true);
        const token = localStorage.getItem('token');
        const res = await fetch(getApiUrl('inventory/recipes'), {
          headers: {
        ...
        const recipes = (body.data || body || []) as any[];
        const recipe = recipes.find(r => (r.menuItemId?._id || r.menuItemId) === item._id);
        if (!recipe || !Array.isArray(recipe.ingredients)) { setEstimatedPours(null); return; }
        let minPours: number | null = null;
        for (const ing of recipe.ingredients) {
          const ingObj = ing.ingredientId || {};
          const trackBy = ingObj.trackBy;
          const vpu = ingObj.volumePerUnitMl;
          const required = ing.quantity; // assume ml per serving
          const stockUnits = ingObj.currentStock;
          if (trackBy === 'volume' && vpu && required && required > 0 && stockUnits > 0) {
            const pours = Math.floor((stockUnits * vpu) / required);
            minPours = minPours === null ? pours : Math.min(minPours, pours);
          }
        }
        setEstimatedPours(minPours);
      } catch (_) {
        // ignore errors
      } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast({
          title: "Success",
          description: data.message || "Menu item updated successfully"
        });
        onSuccess();
      } else {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <Card>
          <CardHeader>
            ...
            {loadingPours ? (
              <p className="text-xs text-muted-foreground">Estimating pours...</p>
            ) : estimatedPours !== null ? (
              <p className="text-xs text-muted-foreground">Estimated pours remaining: <span className="font-medium">{estimatedPours}</span></p>
            ) : null}
          </CardHeader>
