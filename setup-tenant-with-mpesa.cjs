...
async function setupTenantWithMPesa(restaurantKey = 'mama-lucys') {
  ...
  try {
    ...
    let tenant;
    try {
      const tenantResponse = await axios.post('http://localhost:5001/api/superadmin/tenants', tenantData, { headers });
      ...
      if (error.response?.data?.error?.includes('duplicate') || error.response?.data?.error?.includes('exists')) {
        ...
        const existingResponse = await axios.get('http://localhost:5001/api/superadmin/tenants', { headers });
    ...
    await axios.put(`http://localhost:5001/api/payments/config/${tenant._id}`, mpesaConfig, { headers });
    console.log('✅ M-Pesa configuration saved successfully\n');
    
    // Step 4: Verify the configuration
    console.log('4️⃣ Verifying payment configuration...');
    const verifyResponse = await axios.get(`http://localhost:5001/api/payments/config/${tenant._id}`, { headers });
    ...
    const methodsResponse = await axios.get(`http://localhost:5001/api/payments/methods/${tenant._id}`, { headers });
    ...
    try {
      await axios.post('http://localhost:5001/api/superadmin/users', adminUserData, { headers });
...
async function listTenantsWithMPesaStatus() {
  ...
  try {
    ...
    for (const tenant of tenants) {
      ...
      try {
        const configResponse = await axios.get(`http://localhost:5001/api/payments/config/${tenant._id}`, { headers });
